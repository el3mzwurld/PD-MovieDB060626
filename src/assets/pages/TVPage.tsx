...
import type { CastMember, TVShowDetailsFinal, Video } from "../types/appTypes";
import { useEffect, useState } from "react";
import { format } from "date-fns";
const TVPage = () => {
  const { id } = useParams();
  const theme = useTheme();
  const { details, isLoading, error } = useTVDetails(Number(id));
  const navigate = useNavigate();
  const [trailer, setTrailer] = useState<Video | null>(null);

  const findTrailer = (results: TVShowDetailsFinal) => {
    if (!results) return;
  ...
  const formattedDate = details?.first_air_date
    ? format(new Date(details.first_air_date), "MMMM dd, yyyy")
    : "TBA";
